...
class MiembroHogarServiceTest {
    ...
    void setUp() {
        ...
        rolMadre = Rol.builder()
                .nombre("Madre")
                .build();

        rolTutor = Rol.builder()
                .nombre("Tutor")
                .build();
    ...
    @Test
    @DisplayName("Eliminar miembro que NO es admin - Solo elimina")
    void eliminarMiembro_NoEsAdmin_SoloElimina() {
        ...
        when(miembroHogarRepository.findById(1L))
        ...
        verify(miembroHogarRepository, never())
                .findByHogarId(anyLong());
    }

    @Test
    @DisplayName("Eliminar admin - Reasigna Madre como nueva admin")
    void eliminarMiembro_EsAdmin_ReasignaMadre() {
        ...
        when(miembroHogarRepository.findByHogarId(1L))
                .thenReturn(List.of(posibleAdmin));

        miembroHogarService.eliminarMiembro(1L);

        assertTrue(posibleAdmin.getEsAdministrador());

        verify(miembroHogarRepository)
                .save(posibleAdmin);
    }

    @Test
    @DisplayName("Si no hay Madre, debe elegir Tutor")
    void buscarPorPrioridad_EligeTutor() {
    ...
    @Test
    @DisplayName("Si no existe el miembro, lanza excepción")
    void eliminarMiembro_NoEncontrado_LanzaExcepcion() {

        when(miembroHogarRepository.findById(99L))
                .thenReturn(Optional.empty());

        assertThrows(
                BadRequestException.class,
                () -> miembroHogarService.eliminarMiembro(99L)
        );
    }

    @Test
    @DisplayName("Si no quedan miembros, no reasigna admin")
